...
const LeaveApprovalPage = () => {
  ...
  const confirmAction = async () => {
    try {
      const processData = {
        ...
        rejectReason: modalAction === 'reject' 
  ...
  const filteredRequests = leaveRequests.filter(request => {
    ...
    const employeeName = request.full_name || `ID: ${request.user_id}`;
    const matchesSearch = 
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          {request.avatarURL ? (
                            ...
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-medium ${getAvatarColor(request.full_name)}`}>
                              {getInitials(request.full_name || 'Unknown User')}
                            </div>
                          )}
                          <div className="ml-3">
                            <p className="text-sm font-medium text-gray-900">
                              {request.full_name || `ID: ${request.user_id}`}
                            </p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              ...
              <div className="mb-4 bg-gray-50 p-4 rounded-lg">
                <div className="mb-2">
                  <span className="font-medium">Employee:</span> {selectedRequest.full_name} 
                </div>
                <div className="mb-2">
                  <span className="font-medium">Leave Type:</span> {selectedRequest.leave_type}
                </div>
                <div className="mb-2">
                  <span className="font-medium">Period:</span> {new Date(selectedRequest.start_date).toLocaleDateString('en-GB')} to {new Date(selectedRequest.end_date).toLocaleDateString('en-GB')}
                </div>
                <div className="mb-2">
                  <span className="font-medium">Reason:</span> {LEAVE_REASONS[selectedRequest.reason]?.label || selectedRequest.reason}
                </div>
                {selectedRequest.reason === 'OTHER' && selectedRequest.custom_reason && (
                  <div className="mb-2">
                    <span className="font-medium">Custom Reason:</span> {selectedRequest.custom_reason}
                  </div>
                )}
              </div>
              ...
              {modalAction === 'reject' && (
                <div className="mb-4">
                  <label className="block text-gray-700 mb-2">Select Rejection Reason:</label>
                  <select
                    value={rejectReason}
                    onChange={(e) => {
                      setRejectReason(e.target.value);
                      setShowCustomReasonInput(e.target.value === 'OTHER');
                    }}
